...
class CommentWizard(models.TransientModel):
    ...
    validate_reject = fields.Char(store=True)
    res_model = fields.Char()
    res_id = fields.Integer()
    review_ids = fields.Many2many(comodel_name="tier.review")
    comment_id = fields.Many2one(comodel_name='smartest.message.reason', string="Comment")

    comment = fields.Char(related="comment_id.name", store=True)
