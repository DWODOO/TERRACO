...
class TierValidation(models.AbstractModel):
    ...
    _tier_validation_buttons_xpath = "//button[@name='action_rfq_send']"
    ...
    def write(self, vals):
        for rec in self:
            ...
            if (
                    ...
                    and not rec._check_allow_write_under_validation(vals) and any(
                line.status != 'approved' for line in rec.review_ids)
            ):
        ...
        for rec in self:
            if getattr(rec, self._state_field) in self._state_from:
                if rec.need_validation:
                    ...
                    self._update_counter(type="create")
    ...
    def restart_validation(self):
        for rec in self:
            if getattr(rec, self._state_field) in self._state_from:
                ...
                self._update_counter(type='none')
            rec._notify_restarted_review()

    @api.model
    def _update_counter(self,type='done'):
        reviews = self.env["tier.review"].search([('model', '=', self._name), ('res_id', '=', self.id)])
        reviews._compute_can_review(reviews)
        notifications = []
        channel = "base.tier.validation"
        notifications.append([self.env.user.partner_id, channel, type])
        # notifications.append([self.env.user.partner_id,'mail.activity/updated', {'activity_created': True}])
    ...
    def _get_view(
        ...
        if view_type == "form":
            ...
            for node in arch.xpath("/form/header[1]"):
                # By default, after the last button of the header
                node.addnext(etree.fromstring(IrQweb._render(
                    "base_tier_validation.tier_validation_buttons", params
                )))
            for node in arch.xpath("/form/sheet"):
                # node.addnext(etree.fromstring(IrQweb._render(
                #     "base_tier_validation.tier_validation_reviews", params
                # )))
                node.addprevious(etree.fromstring(IrQweb._render(
            ...
            # if view_id and arch.get("base_model", self._name) != self._name:
            #     View = View.with_context(base_model_name=arch["base_model"])
            # res = arch, view
            # new_arch, new_fields = View.postprocess_and_fields(arch, self._name)
            # arch = new_arch
