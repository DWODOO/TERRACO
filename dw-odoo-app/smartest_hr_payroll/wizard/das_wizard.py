...
path = pwd.getpwuid(os.getuid())[5] + '/extractions/' if 'extractions' in os.listdir(pwd.getpwuid(os.getuid())[5]) else \
    os.mkdir(pwd.getpwuid(os.getuid())[5]+"/extractions")


def get_correct_value(value, length_value):
    """This function has been used like this in order to optimize the code"""
...
class DasCnas(models.TransientModel):
    ...
    def get_worked_time_by_trimester(self, payslips, employee_id, trimester, next_trimester):
        ...
        if len(employee_trimester.worked_days_line_ids) == 1:
            ...
            absence_id = self.env.ref('smartest_hr_payroll.hr_absence_days_work_entry').id
