...
class WizardDac(models.TransientModel):
    ...
    def def_employe_paie(self, path):
        ...
        worksheet.write('F9', 'Mouvement', bold)

        rub_paie = self.env['hr.payslip'].search(
            [('date_from', 'like', date_jan), ('state', '=', 'paid')])
        ...
        employee = self.env['hr.contract'].search(
            ['|', ('date_start', 'like', date_jan), ('date_end', 'like', date_jan)])
        print(employee)
        for i in employee:
    ...
    def init_dac(self):
        ...
        action = self.env.ref('l10n_dz_payroll.wizard_dac_action').sudo().read()[0]
